...
def crawl_web_uda(seed, max_depth):
    crawl = []
    tocrawl = [seed]
    crawled = []
    while tocrawl and max_depth >= 0:
        page = tocrawl.pop()
        if page not in crawled:
            union(crawl, get_all_links(get_page(page)))
            crawled.append(page)
        if not tocrawl:
            tocrawl, crawl = crawl, []
            max_depth -= 1
...
def crawl_link(crawled, tocrawl, max_depth=0, depth=0):
    crawldepth = []
    for e in tocrawl:
        if e not in crawled:
            union(crawldepth, get_all_links(get_page(e)))
            crawled.append(e)
            if depth < max_depth and tocrawl.index(e)+1 == len(tocrawl):
                depth += 1
                crawl_link(crawled, crawldepth, max_depth, depth)

def crawl_web(seed, max_depth=0):
    crawled = []
    crawl_link(crawled, [seed], max_depth)
    return crawled
